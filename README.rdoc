...
Take a peek at the plain.html template to see how to load in a Document. All you really need to do is point to a Document Viewer JSON file and specify the target element to embed the viewer:
  ...
  DV.load(JSON_FILE, { hidePageOverflow: true|false, container: element_selector, embedded: true|false });
...
* *hidePageOverflow*: If true, a stylesheet will be added that will hide the page scrollbars. 
* *container*: The element to contain the Document Viewer - '#DV-container'
* *embedded*: If true, it will hide the overflow of the page and expand to the container element constraints
